...
public class Discriminators extends DiscriminatorAndLike {
	
	
	/*-----CINEMA-----*/
	
	/*Update allCinemaLikeGenre hashmap */
	public static HashMap<String,Double> getAllCinemaLikeGenre(Like oneLike, HashMap<String, Double> allCinemaLikeGenre){
		HashMap<String, Double> CinemaLikeGenre = null;
		String oneLikeStr = oneLike.getName();
		CinemaLikeGenre = downloadCinemaLikeGenre(oneLikeStr);	
	...
	/*Search like on IMDB*/
	private static HashMap<String, Double> downloadCinemaLikeGenre(String like){
	...
	public static Double helpCinemaGenreRanking(HashMap<Integer,String> eventGenre, Double rank, Integer eventId, HashMap <String,Double> allCinemaLikeGenre){
		String genre = eventGenre.get(eventId);
		Double ujrank = null;
		if(genre != null){
			...
			Double regiArany = 0.0;
				for(int j = 0; j< parts.length; j=j+2){
					...
					engGenre = hunGenreToEngGenre(parts[j].toLowerCase());
	...
	public static HashMap<String,Double> getAllMusicLikeGenre(Like oneLike, HashMap<String,Double> allMusicLikeGenre){
		List<String> musicLikeGenre = null;
		String oneLikeStr = oneLike.getName();
		try {
			musicLikeGenre = downloadMusicGenreFromLastFm(oneLikeStr);
		} catch (IOException e) {
			// TODO Auto-generated catch block
			//e.printStackTrace();
	...
	/*LAST FM API Key: ad9afb49ea6463b08d4a1107f55ff3d1
	        Secret: is b104f833b9f4283a259000290973a241    brought to you by david*/
	public static List<String> downloadMusicGenreFromLastFm(String artist) throws IOException, JSONException{
		List<String> genres = new ArrayList<String>();
		String method = "artist.gettoptags";
	 	String artisVar = artist.replaceAll(" ", "+");
	 	String apiKey = "ad9afb49ea6463b08d4a1107f55ff3d1";
	 	Integer minNumber = 20;
	...
	public static Double helpMusicGenreRanking(HashMap<Integer,String> eventGenre, Double rank, Integer eventId, HashMap <String,Double> allMusicLikeGenre){
		String genre = eventGenre.get(eventId);
		Double ujrank = null;
		if(genre != null){
			String[] parts = genre.split(",");		
			Double regiArany = 0.0;
				for(int j = 0; j< parts.length; j++){
					if(allMusicLikeGenre.containsKey(parts[j].toLowerCase())){
						Double num = allMusicLikeGenre.get(parts[j].toLowerCase());		 					
